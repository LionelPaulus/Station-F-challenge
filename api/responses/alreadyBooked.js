 ...
 * Example usage:
 * ```
 *     return res.alreadyBooked();
 *     // -or-
 *     return res.alreadyBooked(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function alreadyBooked(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.alreadyBooked()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.alreadyBooked()` called with an Error:', optionalData);
